...
pQuery.group = function(parselet) {
  jQuery.each(parselet, function(key, value) {    
    if(jQuery.isArray(value) && typeof(value[0]) == "object") {
...
pQuery.simplify = function(parselet) {
  jQuery.each(parselet, function(key, value) {
    if(jQuery.isArray(value)) { 
      parselet[key] = value = pQuery.extract(value);
    } else if(value instanceof jQuery) {
      parselet[key] = value = value.extract().simple();
    } else if(value instanceof StringNodeList) {
      parselet[key] = value = value.multiple ? value.simple() : value.simple()[0];
